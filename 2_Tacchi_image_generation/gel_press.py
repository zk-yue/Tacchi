...
sys.path.append('/home/yuezk/yzk/Tacchi/2_Tacchi_image_generation')

# parameters
parser = argparse.ArgumentParser() 
parser.add_argument("--object", default="sphere") # 仿真物体
parser.add_argument("--particle", default="100", choices=["1","10","100"]) # 粒子数量
...
obj_name = "/home/yuezk/yzk/Tacchi/2_Tacchi_image_generation/obj_" + args.particle + "/" + args.object + ".npy"
...
while True:
  ...
  substep()
  if t_ti[None]>=30:
    if (depth-9.5)>-1e-2: # 如果深度值 depth 接近于 11，并且仿真步数是偶数时，程序会保存粒子位置。
      if t_ti[None]%2==0:
        p_xpos_list, p_ypos_list, p_zpos_list = save(p_xpos_list, p_ypos_list, p_zpos_list)
    # print(np.min(p_zpos_list[-1,:]))

    else:
      np.savez('/home/yuezk/yzk/Tacchi/2_Tacchi_image_generation/surface/' + args.particle + '_' +args.object,p_xpos_list=p_xpos_list, p_ypos_list=p_ypos_list, p_zpos_list=p_zpos_list)
      print("press saved")
      sys.exit()
